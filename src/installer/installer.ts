  ...
  async install(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        if (file.size === 0) {
          const originalOptionsHandle = await this.handle.getFileHandle(
            "options.txt"
          );
      ...
      const installPromises = this.mods.map((mod, i) => {
        ...
        return (async () => {
          try {
            const modHandle = await modsHandle.getFileHandle(mod.mod.path, {
              create: true,
            });
            ...
            mod.setState("failed");
            this.logger.error("unable to install mod", { mod: mod.mod, error });
      ...
      if (this.profile.files) {
        for (const [path, url] of Object.entries(this.profile.files)) {
          try {
            ...
            this.logger.error("unable to install file", { path, url });
            this.setState("failed");
            return;
          }
        }
      ...
      const serversHandle = await instanceHandle.getFileHandle("servers.dat", {
        create: true,
      });
  ...
  async installFile(
    ...
    if (segments.length > 1) {
      for (let i = 0; i < segments.length - 1; i++) {
        root = await root.getDirectoryHandle(segments[i], { create: true });
      }
    }

    const fileHandle = await root.getFileHandle(segments[segments.length - 1], {
      create: true,
    });
  ...
  async fetchWithRetry(
    ...
    for (let i = 0; i < 3; i++) {
      try {
        ...
        this.logger.error(`fetch failed (attempt: #${i + 1}), retrying...`, {
          input,
          error,
        });
      }
