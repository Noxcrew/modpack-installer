...
const ModsInstructions = () => {
  ...
  return (
    <>
      ...
      <li>
        Download the mods and extract the contents of <b>mods.zip</b> into the
        mods folder
      </li>
    </>
  );
  ...
  return (
    <div className="invisible lg:visible absolute right-8 bottom-8 p-6 bg-neutral-800 rounded-lg shadow-lg max-w-[26rem]">
      ...
      <p className="text-neutral-300">
        Your browser may support our automatic web-based installer. Getting
        ready is just a few clicks away.
      </p>
      <div>
        <Link
          type="button"
    ...
    <>
      ...
      <div className="px-8 py-12 lg:px-24 lg:py-16">
        ...
        <ul className="text-xl text-neutral-200 list-decimal list-inside mt-8 space-y-2">
          ...
          <li>
            Open the Minecraft Launcher and launch the installed profile called{" "}
            <b>(innit.gg) {profile.name}</b>
          </li>
          {profile.servers.length !== 0 && (
            <li>Add the server address(es) below to your server list</li>
          )}
        </ul>
        <h1 className="mt-4">
